...
## L2/Tiled collections

Virtualizing Level 2 (L2) tiled satellite collections exposes significant limitations when attempting to unify datasets that span multiple native Coordinate Reference Systems (CRSs) or complex overlapping grids.
...
They cannot natively represent heterogeneous, multi-CRS tiles (such as global Sentinel-2 or Landsat grids) without forcing a permanent reprojection to a common global grid — a process that introduces data loss, edge artifacts, and spatial distortion.
...
Another potential follow up topic for L2 would be to investigate the use of [global discrete grids](https://ecce.esri.ca/blog/2023/06/24/introduction-to-discrete-global-grid-system-dggs/) for future missions so lower level data products enable spatial and temporal alignment that will facilitate data processing and the generation of virtual data cubes.
