...
`earthaccess` currently supports data virtualization (via virtualizarr) that can be used to streamline the creation and access of virtual stores.
The current implementation allows end-users and data providers to generate a virtual store at scale with a single line of code:
...
By using CMR search results we could for example, create tailored virtual stores e.g only certain files that meet certain criteria, seasonal datasets (by searching and including only data from a region or temporal range etc.)


Once a virtual store is created, updating it will also be streamlined with `earthaccess`, the store(s) metadata will be listed in a CMR collection level record or a new UMM association,
